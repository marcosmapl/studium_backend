    ...
    verifyToken(req, res, next) {
        try {
            ...
            if (!authHeader) {
                ...
                return res.status(HttpStatus.UNAUTHORIZED).json({ error: "Token não fornecido" });
            }
            ...
            if (parts.length !== 2) {
                ...
                return res.status(HttpStatus.UNAUTHORIZED).json({ error: "Erro no token" });
            }
            ...
            if (!/^Bearer$/i.test(scheme)) {
                ...
                return res.status(HttpStatus.UNAUTHORIZED).json({ error: "Token mal formatado" });
            }
            ...
            jwt.verify(token, JWT_SECRET, (err, decoded) => {
                if (err) {
                    ...
                    return res.status(HttpStatus.UNAUTHORIZED).json({ error: "Token inválido ou expirado" });
                }
            ...
            return res.status(HttpStatus.UNAUTHORIZED).json({ error: "Token inválido" });
        }
    },
};
